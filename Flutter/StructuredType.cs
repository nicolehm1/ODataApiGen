...
{
    public abstract class StructuredType : FlutterRenderable, ILiquidizable
  {
    ...
    public string EntitySetName => Program.Metadata.Schemas.SelectMany(s => s.EntityContainers).SelectMany(c => c.EntitySets).FirstOrDefault(s => s.EntityType == EdmStructuredType.NamespaceQualifiedName)?.Name;
